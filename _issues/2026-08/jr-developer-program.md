---
issue: 2026-08
order: 202608-15
layout: article
author: RANews
title: Jr. Developer Program
toc: false
---

Check out recent sets created by our Junior Developers. Consider the [Junior Developer Program](https://docs.retroachievements.org/developer-docs/jr-dev-rules.html#junior-developer-program-rules) if you are interested in learning how to create achievement sets.

# Jr. Dev Sets <!-- ⭐ -->

| Grad | Set                                                                              | Jr. Developer                   | Reviewer                         | Review Date |
| ---- | -------------------------------------------------------------------------------- | ------------------------------- | -------------------------------- | ----------- |
|      | {% ragamepic 12565, 124471, Wonder Boy: The Dragon's Trap %}                     | {% rauserpic Halbarad %}        | {% rauserpic Salva64bits %}      | 7/1/2026    |
|      | {% ragamepic 23940, 075200, VeggieTales: LarryBoy and the Bad Apple %}           | {% rauserpic RickAlbuquerque %} | {% rauserpic pinguupinguu %}     | 7/1/2026    |
|      | {% ragamepic 20374, 127026, Geometry Wars: Galaxies %}                           | {% rauserpic zeapoz %}          | {% rauserpic zaydoc %}           | 7/1/2026    |
|      | {% ragamepic 17236, 125563, FloWooooT %}                                         | {% rauserpic gluedDown %}       | {% rauserpic Pilzkopf %}         | 7/6/2026    |
|      | {% ragamepic 12853, 126698, Space Bust-A-Move %}                                 | {% rauserpic ZeeRA %}           | {% rauserpic WheelsAndWhirls %}  | 7/10/2026   |
| ⭐    | {% ragamepic 1699, 122487, G.I. Joe: The Atlantis Factor %}                      | {% rauserpic marshallward %}    | {% rauserpic Homuki %}           | 7/14/2026   |
|      | {% ragamepic 17195, 125479, Championship Motocross featuring Ricky Carmichael %} | {% rauserpic vulkansnake %}     | {% rauserpic Pilzkopf %}         | 7/14/2026   |
|      | {% ragamepic 22150, 086594, Touch Detective II ½ %}                              | {% rauserpic Advent %}          | {% rauserpic CySlaytor %}        | 7/21/2026   |
|      | {% ragamepic 22146, 147051, Megamind: The Blue Defender %}                       | {% rauserpic blobdash %}        | {% rauserpic CySlaytor %}        | 7/21/2026   |
|      | {% ragamepic 23801, 147053, ~Homebrew~ Hey You, Pikachu! Demake %}               | {% rauserpic angenoide %}       | {% rauserpic Goodbite %}         | 7/28/2026   |
|      | {% ragamepic 7560, 131031, Bratz: Rock Angelz %}                                 | {% rauserpic MrDeathxxx %}      | {% rauserpic CySlaytor %}        | 7/28/2026   |
| ⭐    | {% ragamepic 7443, 126591, Contra: Legacy of War %}                              | {% rauserpic Sniwott %}         | {% rauserpic zaydoc %}           | 7/28/2026   |
|      | {% ragamepic 7196, 129635, Animetic Story Game 1: Cardcaptor Sakura %}           | {% rauserpic Yzips %}           | {% rauserpic CySlaytor %}        | 7/28/2026   |
|      | {% ragamepic 1811, 126777, M.U.L.E. %}                                           | {% rauserpic Rubicate %}        | {% rauserpic pinguupinguu %}     | 7/29/2026   |
|      | {% ragamepic 21184, 119401, Worms 3D %}                                          | {% rauserpic Wormi %}           | {% rauserpic Souzooka %}         | 7/29/2026   |
|      | {% ragamepic 9771, 075085, The Faery Tale Adventure %}                           | {% rauserpic ShynaNeraShyna %}  | {% rauserpic lonegraywolf2000 %} | 7/30/2026   |
| ⭐    | {% ragamepic 20951, 139707, E.O.E: Eve of Extinction %}                          | {% rauserpic BacktoEdgar %}     | {% rauserpic TheMysticalOne %}   | 7/31/2026   |

# Reviewer Stats

Alongside the Junior Developers are the Code Reviewers, with their never ending quest to ensure the skill set of the incoming juniors meets the necessary expectations. Below you can find some review stats from last month.

|                                 |     |
| ------------------------------- | --- |
| Reviews Requested               | 20  |
| Reviews Completed               | 17  |
| Reviews In Progress             | 17  |
| Jr. Developers Promoted         | 3   |
| Jr. Developers Promoted in 2025 | 22  |
| Active Jr. Developers           | 144 |

<br>

| Reviewer                         | Sets Reviewed |
| -------------------------------- | ------------- |
| {% rauserpic CySlaytor %}        | 4             |
| {% rauserpic pinguupinguu %}     | 2             |
| {% rauserpic zaydoc %}           | 2             |
| {% rauserpic Pilzkopf %}         | 2             |
| {% rauserpic Salva64bits %}      | 1             |
| {% rauserpic WheelsAndWhirls %}  | 1             |
| {% rauserpic Homuki %}           | 1             |
| {% rauserpic Goodbite %}         | 1             |
| {% rauserpic Souzooka %}         | 1             |
| {% rauserpic lonegraywolf2000 %} | 1             |
| {% rauserpic TheMysticalOne %}   | 1             |