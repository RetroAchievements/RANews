---
issue: 2026-05
order: 202605-13
layout: article
title: RAPodcast
author: TimeCrush
---

# Episode 31 -  State of RA

Though I say it nearly every episode, I continue to make it a point that our playerbase and contributors grow day by day, month by month - each new feature or accessibility implementation that improves the quality of life for players and developers alike, bring them in ever growing droves. 

It is no easy task to undertake the responsibility that is implementing sweeping upgrades for such a beloved community. You need to pilot with focus, solid judgement, and a can-do skillset that balances both community input, and a grand vision for the entire ship.

We last spoke to my guest while we were just on the cusp of Dolphin, and in those three years challenges have been overcome, as well as fresh ideas born and raised - It begs the question: What does the future hold for RetroAchievements? 

For exactly that reason, I have my guest here tonight, {% rauserpic WCopeland %} - To dive in on where we were, how far we've come, and what the captain sees himself on the horizon of our future. Much like a "State of the Union" address, this is RetroAchievements "State of RA"!

<p align="center">
    <iframe
        width="640"
        height="480"
        src="https://www.youtube.com/embed/oa7lZ9dRHi0"
        frameborder="0"
        allow="autoplay; encrypted-media"
        allowfullscreen
    >
    </iframe>
</p>

[Spotify Link](https://open.spotify.com/episode/0c7WOPdU8BxqGm8OiBKExX?si=f63fa114cd3a4361)

---

All podcasts can be found on the official [RetroAchievements Podcast YouTube page](https://www.youtube.com/channel/UCI8xnJhIZ2RDf9SEoAx2jFQ). These projects wouldn't exist without the help of quality people like you! If you have an idea for a topic, would like to be featured on the show, or want to send some mail: please write to retroachievementspodcast@gmail.com!