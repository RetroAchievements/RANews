...
class RAHub < Liquid::Tag
  ...
  def render(context)
    ...
    "<a href=\"https://retroachievements.org/hub/#{@id}\" target=\"_blank\" rel=\"noopener\">#{@title}</a>"
  end
  
  Liquid::Template.register_tag 'rahub', self
end

class RAHubPic < Liquid::Tag
  def initialize(tag_name, args, tokens)
  super
    @args = args.split(',', 3).map(&:strip)
  end
  ...
  def render(context)
    ...
    "<a class=\"gameicon-link\" href=\"https://retroachievements.org/hub/#{@id}\" target=\"_blank\" rel=\"noopener\"> <img class=\"gameicon\" src=\"https://retroachievements.org/Images/#{@pic}.png\" > <span>#{@title}</span></a>"
  end
  
  Liquid::Template.register_tag 'rahubpic', self
end
